...
def barchart_selection(main_df, side, side_pos, dte, tick, rate, percentage_array, multiplier):

    main_df = main_df[main_df['Type'] == side][:-1]
    ...
    print(main_df[['strike']])
    main_df[['strike', 'last', 'iv']] = main_df[['strike', 'last', 'iv']].astype('float')
    main_df['side'] = side
    main_df['count'] = -2
    main_df['prime'] = main_df['last']
    main_df['days_to_exp'] = dte

    yahoo_data = yf.download(tick, progress=False)['2018-01-01':]
    underlying = yahoo_data['Close'].iloc[-1]
    main_df['underlying'] = underlying

    estimated_vol = gat_estimated_vol(yahoo_data)
    print('estimated_vol', estimated_vol)

    if side == 'Put':
        if side_pos == 'Long':
            main_df = main_df[main_df['strike'] <= underlying * 1.2].reset_index(drop=True)
            main_df = main_df[main_df['strike'] >= underlying * 0.8].reset_index(drop=True)
        else:
            main_df = main_df[main_df['strike'] <= underlying * 1].reset_index(drop=True)
            main_df = main_df[main_df['strike'] >= underlying * 0.9].reset_index(drop=True)

    else:
        if side_pos == 'Long':
            main_df = main_df[main_df['strike'] <= underlying * 1.2]
            main_df = main_df[main_df['strike'] <= underlying * 0.8]
        else:
            main_df = main_df[main_df['strike'] <= underlying * 1.1].reset_index(drop=True)
            main_df = main_df[main_df['strike'] >= underlying * 1].reset_index(drop=True)
    ...
    for num, row in main_df.iterrows():
        row['symbol'] = tick
        row['rate'] = rate
        row['percentage_array'] = percentage_array
