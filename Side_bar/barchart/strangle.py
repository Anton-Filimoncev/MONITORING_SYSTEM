...
def get_rel_price_76(quotes_put, quotes_call, current_price, u_dte):
    ...
    finish_call = local_df_c['last'] - local_df_c['atm_price']
    finish_call.index = [local_df_c['strike'].values.tolist()]
    finish_put = local_df_p['last'] - local_df_p['atm_price']
    finish_put.index = [local_df_p['strike'].values.tolist()]

    return finish_put, finish_call


def barchart_selection(main_df, dte, tick, rate, percentage_array, multiplier):

    put_df = main_df[main_df['Type'] == 'Put'][:-1]
    put_df.columns = put_df.columns.str.lower()
    put_df['strike'] = put_df['strike'].str.replace('-', '.').replace('s', '')
    # short_df['last'] = short_df['last'].str.replace('-', '.').replace('s', '')
    print(put_df)
    put_df['iv'] = put_df['iv'].str.replace('%', '')
    print(put_df[['strike', 'last', 'iv']])
    put_df[['strike', 'last', 'iv']] = put_df[['strike', 'last', 'iv']].astype('float')
    put_df['side'] = 'Put'
    put_df['count'] = -1
    put_df['prime'] = put_df['last']
    put_df['days_to_exp'] = dte


    call_df = main_df[main_df['Type'] == 'Call'][:-1]
    call_df.columns = call_df.columns.str.lower()
    call_df['strike'] = call_df['strike'].str.replace('-', '.').replace('s', '')
    # long_df['last'] = long_df['last'].str.replace('-', '.').replace('s', '')
    call_df['iv'] = call_df['iv'].str.replace('%', '')
    call_df[['strike', 'last', 'iv']] = call_df[['strike', 'last', 'iv']].astype('float')
    call_df['side'] = 'Call'
    call_df['prime'] = call_df['last']
    call_df['count'] = -1
    call_df['days_to_exp'] = dte

    yahoo_data = yf.download(tick, progress=False)['2018-01-01':]
    underlying = yahoo_data['Close'].iloc[-1]
    put_df['underlying'] = underlying
    call_df['underlying'] = underlying

    estimated_vol = gat_estimated_vol(yahoo_data)

    exp_move = estimated_vol * underlying * math.sqrt(dte / 365)
    ...
    delta_5_put = nearest_equal_abs(put_df['delta'].dropna().abs().values.tolist(), 0.05)
    delta_10_put = nearest_equal_abs(put_df['delta'].dropna().abs().values.tolist(), 0.1)
    delta_15_put = nearest_equal_abs(put_df['delta'].dropna().abs().values.tolist(), 0.15)

    delta_5_call = nearest_equal_abs(call_df['delta'].dropna().abs().values.tolist(), 0.05)
    delta_10_call = nearest_equal_abs(call_df['delta'].dropna().abs().values.tolist(), 0.1)
    delta_15_call = nearest_equal_abs(call_df['delta'].dropna().abs().values.tolist(), 0.15)


    finish_put, finish_call = get_rel_price_76(put_df, call_df, underlying, dte)
    max_put_skew_strike = finish_put[finish_put == finish_put.max()].index.tolist()[0][0]
    skew_delta_put = put_df[put_df['strike'] == max_put_skew_strike].reset_index(drop=True)['delta'].abs()[0]
    skew_delta_call = nearest_equal_abs(call_df['delta'].dropna().abs().values.tolist(), skew_delta_put)
    ...
    for delta in delta_list:
        delta_put = delta[0]
        delta_call = delta[1]
        df_put_1 = put_df[put_df['delta'].abs() == delta_put].reset_index(drop=True)
        df_call_1 = call_df[call_df['delta'].abs() == delta_call].reset_index(drop=True)
        combined_df = pd.concat([df_put_1, df_call_1], axis=0)
    ...
    for df in combined_dfs:
        ...
        print('````df````')
        print(df)
        print(df[df['side'] == 'Put'])
        response_df['Put_Strike'] = df[df['side'] == 'Put']['strike'].values[0]
        response_df['Call_Strike'] = df[df['side'] == 'Call']['strike'].values[0]
