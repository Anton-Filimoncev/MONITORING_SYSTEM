...
def monteCarlo_exp_return(underlying, start_underlying, rate, sigma, days_to_expiration, closing_days_array, trials, initial_credit,
    ...
    for c in range(trials):
        ...
        for r in range(max_closing_days + 1):
            ...
            debit, stock_debit = bsm_func(stock_price, start_underlying, strikes, rate, dt * (days_to_expiration - r), sigma)

            profit = initial_credit - debit + stock_debit  # Profit if we were to close on current day
