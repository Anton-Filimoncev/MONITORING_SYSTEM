...
def bsm_debit(sim_price, strikes, rate, time_fraction_short, time_fraction_long, sigma_short, sigma_long):
    P_short_cals = blackScholesCall(sim_price, strikes[1], rate, time_fraction_short, sigma_short)
    P_long_cals = blackScholesCall(sim_price, strikes[0], rate, time_fraction_long, sigma_long)

    credit = P_long_cals - P_short_cals
    debit = P_long_cals - P_short_cals
    # debit = P_short_cals - P_long_cals

    return debit, P_short_cals, P_long_cals
...
def callCalendar(underlying, sigma_short, sigma_long, rate, trials, days_to_expiration_short,
    ...
    response = {
        "pop": pop,
        'cvar': cvar,
        'exp_return': expected_profit * 100,
        "pop_error": pop_error,
        "avg_dtc": avg_dtc,
